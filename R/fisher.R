...
#' in levels and changes with Fisher index.
#'
#' The Fisher productivity index is the geometric average of Laspeyres and
#' Paasche indices.
#'
#' Deflated shadow prices of inputs and outputs can also be computed.
#'
#' @aliases fisher print.Fisher
#'
...
#' only positive technological change (i.e. technological progress) is alfisherd.
#' See also the \code{Details} section.
...
#' @param x an object of class \code{Fisher} (returned by the function
#' \code{\link{fisher}}).
...
#'
#' The Fisher index is not transitive and therefore each firm is compared to
...
#' \code{NA}
#'
#' A set of extractor functions for objects of class \code{'Fisher'}
#' including methods to the generic functions
#' \code{\link[=print.Fisher]{print}},
#' \code{\link[=Levels.Fisher]{Levels}},
#' \code{\link[=Changes.Fisher]{Changes}}, and
#' \code{\link[=Shadowp.Fisher]{Shadowp}}.
#'
#' @return \code{fisher()} returns a list of class \verb{'Fisher'}
...
#' Hence, all efficiency scores are greater than zero and are fisherr or equal to
#' one.
#'
#' @author K Hervé Dakpo, Yann Desjeux, Laure Latruffe
#'
#' @section Warning:
#' The \code{fisher()} function will not work with
#' unbalanced panel data. The Fisher index may be sensitive to the
#' rescaling.
#'
...
#'
#' @seealso
#' \code{\link[=print.Fisher]{print}}: for printing summary of each
#' element of the list in the \code{'Fisher'} object;
#'
#' \code{\link[=Levels.Fisher]{Levels}}: for extracting individual
#' productivity and profitability \bold{levels};
#'
#' \code{\link[=Changes.Fisher]{Changes}}: for extracting individual
#' productivity and profitability \bold{change indices};
#'
#' \code{\link[=Shadowp.Fisher]{Shadowp}}: for extracting inputs and
#' outputs \bold{deflated shadow prices}, if \code{shadow = TRUE};
#'
#' \code{\link[=paasche]{paasche}}: for computing the Paasche
#' productivity index;
#'
#' \code{\link[=fisher]{fisher}}: for computing the Laspeyres
#' productivity index.
#'
...
#'
#' Diewert W.E. (1992), Fisher ideal output, input, and productivity indexes
#' revisited. \emph{Journal of Productivity Analysis}, \bold{3}(3), 211-248.
...
#' \dontrun{
#' Fisher.prod <- fisher(data = usagri, id.var = "States", time.var = "Years",
...
We recommend rescaling the data or set the scaled option to TRUE to avoid numerical problems\n\r", call. = FALSE)
  ...
  if (tech.change == TRUE) {
    res.fish.loop <- foreach(ano = 1:(length(year.vec)), .combine = rbind, .packages = c("lpSolveAPI",
      "doParallel"), .export = c("fish.1", "DO.teseme", "DI.teseme", "D.tfp",
                                 "DO.shdu", "DI.shdu")) %dopar% {
      fish.1(data, step1, ano, year.vec, tech.reg, rts, orientation, parallel, mean.x, mean.y, itt, it, shadow)
    }
  } else {
    res.fish.loop <- foreach(ano = 1:(length(year.vec)), .combine = rbind, .packages = c("lpSolveAPI",
      "doParallel"), .export = c("fish.2", "DO.teseme", "DI.teseme", "D.tfp",
                                 "DO.shdu", "DI.shdu")) %dopar% {
      fish.2(data, step1, ano, year.vec, rts, orientation, parallel, mean.x, mean.y, itt, it, shadow)
    }
  }
  res.fish.loop <- cbind(data[, c(step1$id.var, step1$time.var)], res.fish.loop)
  row.names(res.fish.loop) <- seq(1:dim(res.fish.loop)[1])
    indices <- foreach(ano = 2:(length(year.vec)), .combine = rbind, .packages = c("doParallel"),
      .export = c("fdiv")) %dopar% {
      cbind(res.fish.loop[res.fish.loop[, 2] == year.vec[ano], c("REV", "COST", "PROF")]/res.fish.loop[res.fish.loop[,
        2] == year.vec[ano - 1], c("REV", "COST", "PROF")], res.fish.loop[res.fish.loop[,
        2] == year.vec[ano], c("P", "W")]/(res.fish.loop[res.fish.loop[, 2] == year.vec[ano -
        1], c("REV", "COST")]/res.fish.loop[res.fish.loop[, 2] == year.vec[ano],
        c("Qs", "Xs")]), TT = res.fish.loop[res.fish.loop[, 2] == year.vec[ano],
        "TT"]/fdiv(res.fish.loop[res.fish.loop[, 2] == year.vec[ano - 1], c("REV",
        "COST")]/res.fish.loop[res.fish.loop[, 2] == year.vec[ano], c("Qs", "Xs")]),
        res.fish.loop[res.fish.loop[, 2] == year.vec[ano], c("AO", "AI")]/res.fish.loop[res.fish.loop[,
          2] == year.vec[ano], c("Qs", "Xs")], res.fish.loop[res.fish.loop[, 2] ==
          year.vec[ano], c("TFP", "MP", "TFPE")]/res.fish.loop[res.fish.loop[, 2] ==
          year.vec[ano], c("TFP2", "MP2", "TFPE2")], res.fish.loop[res.fish.loop[,
          2] == year.vec[ano], 14:15]/res.fish.loop[res.fish.loop[, 2] == year.vec[ano -
          1], 14:15], res.fish.loop[res.fish.loop[, 2] == year.vec[ano], 16:20]/res.fish.loop[res.fish.loop[,
          2] == year.vec[ano], 28:32])
    }
    registerDoSEQ()
    stopImplicitCluster()
    INT <- matrix(nrow = dim(res.fish.loop[res.fish.loop[, 2] == year.vec[1], ])[1],
      ncol = 18)
    INT <- as.data.frame(INT)
    names(INT) <- names(indices)
    indices <- rbind(INT, indices)
    indices <- cbind(res.fish.loop[, 1:2], indices)
    names(indices)[3:20] <- paste0("d", names(res.fish.loop[, 3:20]))
    if (shadow == TRUE) {
      Shadowp <- cbind(data[, c(step1$id.var, step1$time.var)], res.fish.loop[, 33:(dim(res.fish.loop)[2])])
      names(Shadowp) <- c(names(data[id.var]), names(data[time.var]), names(data[x.vars]), names(data[y.vars]))
      res.tfp <- list(Levels = res.fish.loop[, 1:20], Changes = indices, Shadowp = Shadowp)
    } else {
      res.tfp <- list(Levels = res.fish.loop[, 1:20], Changes = indices)
    }
...
print.Fisher <- function(x, digits = NULL, ...) {
  ...
  if (!is.null(x[["Shadowp"]])) {
    cat("\n\nFisher productivity shadow prices:\n\n")
...
# Return Fisher productivity and profitability Levels ----------
#' @rdname fisher
#' @export
Levels.Fisher <- function(x, ...) {
  return(x$Levels)
}

# Return Fisher productivity and profitability Changes ----------
#' @rdname fisher
#' @export
Changes.Fisher <- function(x, ...) {
  return(x$Changes)
}
