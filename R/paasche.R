...
#' in levels and changes with Paasche index.
#'
#' Deflated shadow prices of inputs and outputs can also be computed.
#'
#' @aliases paasche print.Paasche
#'
...
#' only positive technological change (i.e. technological progress) is alpaasched.
#' See also the \code{Details} section.
...
#' @param x an object of class \code{Paasche} (returned by the function
#' \code{\link{paasche}}).
...
#'
#' The Paasche index is not transitive and therefore each firm is compared to
...
#' \code{NA}
#'
#' A set of extractor functions for objects of class \code{'Paasche'}
#' including methods to the generic functions
#' \code{\link[=print.Paasche]{print}},
#' \code{\link[=Levels.Paasche]{Levels}},
#' \code{\link[=Changes.Paasche]{Changes}}, and
#' \code{\link[=Shadowp.Paasche]{Shadowp}}.
#'
#' @return \code{paasche()} returns a list of class \verb{'Paasche'}
...
#' Hence, all efficiency scores are greater than zero and are paascher or equal to
#' one.
#'
#' @author K Hervé Dakpo, Yann Desjeux, Laure Latruffe
#'
#' @section Warning:
#' The \code{paasche()} function will not work with
#' unbalanced panel data. The Paasche index may be sensitive to the
#' rescaling.
#'
...
#'
#' @seealso
#' \code{\link[=print.Paasche]{print}}: for printing summary of each
#' element of the list in the \code{'Paasche'} object;
#'
#' \code{\link[=Levels.Paasche]{Levels}}: for extracting individual
#' productivity and profitability \bold{levels};
#'
#' \code{\link[=Changes.Paasche]{Changes}}: for extracting individual
#' productivity and profitability \bold{change indices};
#'
#' \code{\link[=Shadowp.Paasche]{Shadowp}}: for extracting inputs and
#' outputs \bold{deflated shadow prices}, if \code{shadow = TRUE};
#'
#' \code{\link[=laspeyres]{laspeyres}}: for computing the Laspeyres
#' productivity index;
#'
...
#' \dontrun{
#' Paasche.prod <- paasche(data = usagri, id.var = "States", time.var = "Years",
...
We recommend rescaling the data or set the scaled option to TRUE to avoid numerical problems\n\r", call. = FALSE)
  ...
  if (tech.change == TRUE) {
    res.paa.loop <- foreach(ano = 1:(length(year.vec)), .combine = rbind, .packages = c("lpSolveAPI",
      "doParallel"), .export = c("paa.1", "DO.teseme", "DI.teseme", "D.tfp",
                                 "DO.shdu", "DI.shdu")) %dopar% {
      paa.1(data, step1, ano, year.vec, tech.reg, rts, orientation, parallel, mean.x, mean.y, itt, it, shadow)
    }
  } else {
    res.paa.loop <- foreach(ano = 1:(length(year.vec)), .combine = rbind, .packages = c("lpSolveAPI",
      "doParallel"), .export = c("paa.2", "DO.teseme", "DI.teseme", "D.tfp",
                                 "DO.shdu", "DI.shdu")) %dopar% {
      paa.2(data, step1, ano, year.vec, rts, orientation, parallel, mean.x, mean.y, itt, it, shadow)
    }
  }
  res.paa.loop <- cbind(data[, c(step1$id.var, step1$time.var)], res.paa.loop)
  row.names(res.paa.loop) <- seq(1:dim(res.paa.loop)[1])
    indices <- foreach(ano = 2:(length(year.vec)), .combine = rbind, .packages = c("doParallel"),
      .export = c("fdiv")) %dopar% {
      cbind(res.paa.loop[res.paa.loop[, 2] == year.vec[ano], c("REV", "COST", "PROF")]/res.paa.loop[res.paa.loop[,
        2] == year.vec[ano - 1], c("REV", "COST", "PROF")], res.paa.loop[res.paa.loop[,
        2] == year.vec[ano], c("P", "W")]/(res.paa.loop[res.paa.loop[, 2] == year.vec[ano -
        1], c("REV", "COST")]/res.paa.loop[res.paa.loop[, 2] == year.vec[ano], c("Qs",
        "Xs")]), TT = res.paa.loop[res.paa.loop[, 2] == year.vec[ano], "TT"]/fdiv(res.paa.loop[res.paa.loop[,
        2] == year.vec[ano - 1], c("REV", "COST")]/res.paa.loop[res.paa.loop[, 2] ==
        year.vec[ano], c("Qs", "Xs")]), res.paa.loop[res.paa.loop[, 2] == year.vec[ano],
        c("AO", "AI")]/res.paa.loop[res.paa.loop[, 2] == year.vec[ano], c("Qs", "Xs")],
        res.paa.loop[res.paa.loop[, 2] == year.vec[ano], c("TFP", "MP", "TFPE")]/res.paa.loop[res.paa.loop[,
          2] == year.vec[ano], c("TFP2", "MP2", "TFPE2")], res.paa.loop[res.paa.loop[,
          2] == year.vec[ano], 14:15]/res.paa.loop[res.paa.loop[, 2] == year.vec[ano -
          1], 14:15], res.paa.loop[res.paa.loop[, 2] == year.vec[ano], 16:20]/res.paa.loop[res.paa.loop[,
          2] == year.vec[ano], 28:32])
    }
    registerDoSEQ()
    stopImplicitCluster()
    INT <- matrix(nrow = dim(res.paa.loop[res.paa.loop[, 2] == year.vec[1], ])[1], ncol = 18)
    INT <- as.data.frame(INT)
    names(INT) <- names(indices)
    indices <- rbind(INT, indices)
    indices <- cbind(res.paa.loop[, 1:2], indices)
    names(indices)[3:20] <- paste0("d", names(res.paa.loop[, 3:20]))
    if (shadow == TRUE) {
      Shadowp <- cbind(data[, c(step1$id.var, step1$time.var)], res.paa.loop[, 33:(dim(res.paa.loop)[2])])
      names(Shadowp) <- c(names(data[id.var]), names(data[time.var]), names(data[x.vars]), names(data[y.vars]))
      res.tfp <- list(Levels = res.paa.loop[, 1:20], Changes = indices, Shadowp = Shadowp)
    } else {
      res.tfp <- list(Levels = res.paa.loop[, 1:20], Changes = indices)
    }
...
print.Paasche <- function(x, digits = NULL, ...) {
  ...
  if (!is.null(x[["Shadowp"]])) {
    cat("\n\nPaasche productivity shadow prices:\n\n")
...
# Return Paasche productivity and profitability Levels ----------
#' @rdname paasche
#' @export
Levels.Paasche <- function(x, ...) {
  return(x$Levels)
}

# Return Paasche productivity and profitability Changes ----------
#' @rdname paasche
#' @export
Changes.Paasche <- function(x, ...) {
  return(x$Changes)
}
